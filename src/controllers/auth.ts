...
import { GITHUB_ACCESS_TOKEN_URL, GITHUB_OAUTH_CLIENT_ID,  GITHUB_GET_USER_URL, GITHUB_USER_EMAIL_URL} from "../common/constants"
...
export const registerUser = async (req: Request) => {
    try {
        ...
        const userEmailsResult = await axios.get(
        ...
        if (userEmailsResult.status !== 200 || !Array.isArray(userEmailsResult.data)) {
            return {
                status: 400,
                response: generateAPIResponse({message: 'Something went wrong'})
            }
        ...
        const primaryUserEmailData = userEmailsResult.data.find((emailData) => emailData.primary)
        ...
        const userDataResult = await getUserDataIfExists(primaryUserEmailData.email);

        let userData:any;

        if(!userDataResult.success) {
            const userGithubDataResult = await axios.get(
                GITHUB_GET_USER_URL,
                {
                    headers: {
                        Authorization : `Bearer ${accessToken}`
